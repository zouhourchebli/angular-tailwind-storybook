...
const bindWithContext = (storyFn, getStoryContext) => // (NOTE: You cannot override the parameters key, it is fixed)
(_ref = {}) => {
  let contextUpdate = _objectWithoutPropertiesLoose(_ref, ["id", "name", "kind", "parameters"]);

  return storyFn(Object.assign({}, getStoryContext(), contextUpdate));
};

export const decorateStory = (storyFn, decorator, getStoryContext) => {
...
export const defaultDecorateStory = (storyFn, decorators) => {
  ...
  let contextStore;
  const decoratedWithContextStore = decorators.reduce((story, decorator) => decorateStory(story, decorator, () => contextStore), storyFn);
